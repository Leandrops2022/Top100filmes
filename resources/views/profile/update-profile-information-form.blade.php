<x-form-section submit="updateProfileInformation">
    ...
    <x-slot name="description">
        {{ __('Atualize o nome e endereço de email da sua conta') }}
    </x-slot> --}}
    ...
    {{-- <div class="col-span-6 sm:col-span-4">
            ...
            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && !$this->user->hasVerifiedEmail())
                <p class="text-sm mt-2 dark:text-white">
                    {{ __('Seu dendereço de email ainda não foi verificado.') }}

                    <button type="button"
                        class="underline text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-800"
                        wire:click.prevent="sendEmailVerification">
                        {{ __('Clique aqui para reenviar o link de verificação de email.') }}
                    </button>
                </p>

                @if ($this->verificationLinkSent)
                    <p class="mt-2 font-medium text-sm text-green-400 dark:text-green-400">
    ...
    <x-slot name="actions">
        ...
        <x-secondary-button class="text-gray-400" wire:loading.attr="disabled" wire:target="photo">
            {{ __('Salvar') }}
        </x-secondary-button>
    </x-slot>
</x-form-section>
